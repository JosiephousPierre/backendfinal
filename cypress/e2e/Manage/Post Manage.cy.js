...
describe('API Manage Endpoint Testing', () => {
    ...
    it('should CREATE a student in the manage table', () => {
        ...
        cy.get('.parameters-col_description').first();
                ...
                cy.get('input[type="text"][placeholder="nurseID"]').clear().type('3');

                // Locate the course input field and enter a value
                cy.get('input[type="text"][placeholder="course"]').clear().type('SampleCourse');

                // Locate the student last name input field and enter a value
                cy.get('input[type="text"][placeholder="student_Lname"]').clear().type('Doe');

                // Locate the student first name input field and enter a value
                cy.get('input[type="text"][placeholder="student_Fname"]').clear().type('John');
